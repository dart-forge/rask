...
class FakeRegistry implements PackageRegistry {
  ...
  Future<bool> hasVersion(
      {required String host, required String name, required String version}) async {
...
void main() {
  ...
  test('asks the publish_to host for privately published packages', () async {
    final (_, _, reg, _) = await publish(packages: [ws['internal'], ws['core']]);
    expect(reg.queries, containsAll([('https://pub.example.com', 'internal'), ('https://pub.dev', 'core')]));
  });

  test('a dry run still consults the registry but never skips because of it', () async {
    final reg = FakeRegistry(published: {'https://pub.dev': {'core': {'0.2.0'}}});
    final (_, runner, _, out) = await publish(packages: [ws['core']], dryRun: true, registry: reg);
  ...
  test('a registry failure stops the run instead of skipping', () async {
    ...
    final (code, runner, _, out) = await publish(packages: [ws['core']], registry: reg);
    expect(code, isNot(0));
