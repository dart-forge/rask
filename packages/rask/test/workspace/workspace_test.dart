...
void main() {
  ...
  void writePubspec(String relDir, String yaml) {
    final dir = Directory(p.join(root.path, relDir))
...
''');
  ...
  test('dependents lists packages that depend on a package, transitively', () {
    final ws = Workspace.load(root);
    expect(
      ws.dependentsOf('tmp4').map((pkg) => pkg.name),
  ...
  test('findRoot walks up from a member directory to the workspace root', () {
    final found = Workspace.findRoot(
      Directory(p.join(root.path, 'packages', 'tmp3')),
    );
...
''');
    ...
    test(
      'members of a nested workspace become members of the top-level one',
      () {
        final ws = Workspace.load(root);
        expect(
          ws.packages.map((pkg) => pkg.name),
    ...
    test(
      'findRoot from inside a nested workspace returns the top-level root',
      () {
    ...
    test('findRoot from the nested root itself returns the top-level root', () {
      expect(
        Workspace.findRoot(Directory(p.join(root.path, 'example')))?.path,
        root.path,
      );
    });
  });
  ...
  group(
    'a top-level root nested under another workspace (e.g. a git worktree)',
    () {
      ...
      setUp(() {
        ...
        writePubspec(
          '.claude/worktrees/wt/packages/tmp1',
          'name: tmp1\nresolution: workspace\n',
        );
      });
