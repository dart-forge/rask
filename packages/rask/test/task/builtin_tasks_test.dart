...
class FakeContext implements TaskContext {
  ...
  }) async => calls.add((executable, args));
  @override
...
void main() {
  ...
  setUp(() {
    ...
    Directory(p.join(root.path, 'packages/with_tests/test/unit'))
        .createSync(recursive: true);
    File(p.join(root.path, 'packages/with_tests/test/unit/a_test.dart'))
        .writeAsStringSync('');
    ...
    File(p.join(root.path, 'packages/helper_only/test/helper.dart'))
        .writeAsStringSync('');
  ...
  test(
    'test runs `dart test <args>`; analyze runs `dart analyze <args>`',
    () async {
