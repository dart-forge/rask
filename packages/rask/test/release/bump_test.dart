...
''';
    ...
    test(
      'leaves hosted non-members, path-valued members and unknown names alone',
      () {
        final out = bumpInternalConstraints(src, members, '0.2.0');
        ...
        expect(
          out,
          contains('  aim_server_testing: ^0.1.1\n'),
        ); // not in members
    ...
    test('also rewrites dev_dependencies', () {
      final out = bumpInternalConstraints(src, {
        ...members,
        'aim_server_testing',
      }, '0.2.0');
  ...
  group('foldChangelog', () {
    ...
    test(
      'inserts a heading after the title when there is no Unreleased section',
      () {
        const src = '# Changelog\n\n## 0.1.0\n\n- First.\n';
        expect(
          foldChangelog(src, '0.2.0'),
  ...
  group('bumpWorkspace', () {
    ...
    test('bumps constraints between publishable members', () {
      bumpWorkspace(Workspace.load(root), '0.2.0', StringBuffer());
      expect(
        read('packages/server/pubspec.yaml'),
        contains('  core: ^0.2.0\n'),
      );
    });
    ...
    test('folds each publishable member\'s CHANGELOG', () {
      ...
      expect(
        read('packages/server/CHANGELOG.md'),
    ...
    test('does not create a CHANGELOG where there is none', () {
      ...
      bumpWorkspace(Workspace.load(root), '0.2.0', StringBuffer());
      expect(
        File(p.join(root.path, 'packages/core/CHANGELOG.md')).existsSync(),
        isFalse,
      );
    });
    ...
    test('reports what it changed and returns the changed files', () {
      ...
      expect(
        changed.map((f) => p.relative(f, from: root.path)),
        unorderedEquals([
